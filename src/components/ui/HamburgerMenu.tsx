...
export default function HamburgerMenu({ visible, onClose }: Props) {
  ...
  const go = (screen: string) => {
    ...
    if (['profile', 'activities', 'messages', 'notifications', 'home'].includes(screen.toLowerCase())) {
      router.replace(`/(parent)/${screen.toLowerCase()}` as any);
    } else {
      router.push(`/(parent)/${screen.toLowerCase()}` as any);
    }
  };
            ...
            >
              ...
              >
                <Ionicons
                  ...
                  style={styles.icon}
                />
                <Text style={[styles.text, { color: colors.text }]}>My Bookings</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.item, { borderBottomColor: colors.border }]}
                onPress={() => go('messages')}
              ...
              >
                <Ionicons
                  ...
                  style={styles.icon}
                />
                <Text style={[styles.text, { color: colors.text }]}>Child Profiles</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.item, { borderBottomColor: colors.border }]}
                onPress={() => go('profile')}
