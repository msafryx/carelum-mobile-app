...
export default function SitterNotificationsScreen() {
  ...
  const handleAlertPress = async (alert: Alert) => {
    ...
    if (alert.sessionId) {
      router.push(`/(sitter)/session/${alert.sessionId}` as any);
    }
  };

  // Role-aware message: sitter sees "You accepted/declined/started", not parent-facing wording
  const getDisplayMessage = (alert: Alert): string => {
    if (!user?.id) return alert.message;
    if (alert.type === 'session_accepted' && alert.sitterId === user.id) {
      return 'You have accepted this session request.';
    }
    if (alert.type === 'session_cancelled' && alert.sitterId === user.id) {
      return 'You have declined this invitation.';
    }
    if (alert.type === 'session_started' && alert.sitterId === user.id) {
      return 'You have started the session.';
    }
